...
from .misc import (_compute_error_ratio, _convert_to_tensor,
                   _handle_unused_kwargs, _is_finite, _is_iterable,
                   _optimal_step_size, _scaled_dot_product,
                   _select_initial_step)
from .rk_common import _ButcherTableau, _runge_kutta_step, _RungeKuttaState
...
class Dopri5Solver(AdaptiveStepsizeODESolver):
    ...
    def __init__(
        self, func, y0, f_options, rtol, atol, first_step=None, safety=0.9, ifactor=10.0, dfactor=0.2, max_num_steps=2**31 - 1,
        **unused_kwargs
    ):
    ...
    def before_integrate(self, t):
        ...
        if self.first_step is None:
            first_step = _select_initial_step(self.func, t[0], self.y0, self.f_options, 4, self.rtol[0], self.atol[0], f0=f0).to(t)
        else:
    ...
    def _adaptive_dopri5_step(self, rk_state):
        ...
        y1, f1, y1_error, k = _runge_kutta_step(self.func, y0, f0, t0, dt, tableau=_DORMAND_PRINCE_SHAMPINE_TABLEAU, f_options=self.f_options)
