...
from .misc import (_compute_error_ratio, _convert_to_tensor,
                   _handle_unused_kwargs, _is_iterable, _optimal_step_size,
                   _scaled_dot_product, _select_initial_step)
...
class VariableCoefficientAdamsBashforth(AdaptiveStepsizeODESolver):
    ...
    def __init__(
        self, func, y0, f_options, rtol, atol, implicit=True, max_order=_MAX_ORDER, safety=0.9, ifactor=10.0, dfactor=0.2,
        **unused_kwargs
    ):
    ...
    def before_integrate(self, t):
        ...
        phi.appendleft(f0)
        first_step = _select_initial_step(self.func, t[0], self.y0, self.f_options, 2, self.rtol[0], self.atol[0], f0=f0).to(t)
