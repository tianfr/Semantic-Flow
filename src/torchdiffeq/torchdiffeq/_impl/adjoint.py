...
class OdeintAdjointMethod(torch.autograd.Function):
    ...
    def forward(ctx, *args):
        ...
        y0, func, t, flat_params, rtol, atol, method, options, f_options = \
            args[:-8], args[-8], args[-7], args[-6], args[-5], args[-4], args[-3], args[-2], args[-1]

        ctx.func, ctx.rtol, ctx.atol, ctx.method, ctx.options, ctx.f_options = func, rtol, atol, method, options, f_options

        with torch.no_grad():
            ans = odeint(func, y0, t, rtol=rtol, atol=atol, method=method, options=options, f_options=f_options)
    ...
    def backward(ctx, *grad_output):
        ...
        ans = tuple(ans)
        func, rtol, atol, method, options, f_options = ctx.func, ctx.rtol, ctx.atol, ctx.method, ctx.options, ctx.f_options
        n_tensors = len(ans)
        ...
        with torch.no_grad():
            ...
            for i in range(T - 1, 0, -1):
                ...
                aug_ans = odeint(
                    ...
                    torch.tensor([t[i], t[i - 1]]), rtol=rtol, atol=atol, method=method, options=options, f_options=f_options
                )
...
def odeint_adjoint(func, y0, t, rtol=1e-6, atol=1e-12, method=None, options=None, f_options=None):
    ...
    ys = OdeintAdjointMethod.apply(*y0, func, t, flat_params, rtol, atol, method, options, f_options)
