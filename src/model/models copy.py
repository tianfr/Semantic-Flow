...
class SemanticNet_d(torch.nn.Module):
    def __init__(self, d_hidden_dynamic, semantic_class_num):
        super().__init__()
        self.d_hidden_dynamic = d_hidden_dynamic 
    ...
    def forward(self, weights_d, semantic_latents_d, **kwargs):
        semantic_latent_map_d = torch.sum(weights_d[..., None] * semantic_latents_d, -2)
        # return self.semantic_layers_dynamic( semantic_latent_map_d)
        return semantic_latent_map_d
    
class SemanticNet_s(torch.nn.Module):
    def __init__(self, d_hidden_static, semantic_class_num):
        super().__init__()
        self.d_hidden_static = d_hidden_static
        self.semantic_class_num = semantic_class_num
    ...
    def forward(self, weights_s, semantic_latents_s, **kwargs):
        semantic_latent_map_s = torch.sum(weights_s[..., None] * semantic_latents_s, -2)
        return self.semantic_layers_static(semantic_latent_map_s)
        # return semantic_latent_map_s
...
class PixelNeRFNet(torch.nn.Module):
    def __init__(self, conf, stop_encoder_grad=True, use_static_resnet=False):
        ...
        self.semantic_static = SemanticNet_s(self.d_hidden_static, self.semantic_class_num)
        self.semantic_dynamic = SemanticNet_d(self.d_hidden_dynamic, self.semantic_class_num)
    ...
    # def semantic_d(self, weights_d, semantic_latents_d, **kwargs):
    #     semantic_latent_map_d = torch.sum(weights_d[..., None] * semantic_latents_d, -2)
    #     return self.semantic_layers_dynamic( semantic_latent_map_d)

    # def semantic_s(self, weights_s, semantic_latents_s, **kwargs):
    #     semantic_latent_map_s = torch.sum(weights_s[..., None] * semantic_latents_s, -2)
    #     return self.semantic_layers_static(semantic_latent_map_s)
    
    # def semantic_full(self, T_full, alpha_d, alpha_s, blending, semantic_latents_d, semantic_latents_s, **kwargs):
    #     semantic_latent_map_full = torch.sum(
    #         (T_full * alpha_d * blending)[..., None] * semantic_latents_d + \
    #         (T_full * alpha_s * (1. - blending))[..., None] * semantic_latents_s, -2)
    #     return self.semantic_layers_static(semantic_latent_map_full)
    ...
    def raw2outputs(self,
                ...
                semantic_logits_s,
                semantic_logits_d,
                semantic_latents_s=None,
                semantic_latents_d=None,
                net=None,
                ):
        ...
        if use_semantic_labels:
            ...
            import ipdb; ipdb.set_trace()
            # semantic_latent_map_d = torch.sum(weights_d[..., None] * semantic_latents_d, -2)
            # semantic_latent_map_dyreg_d = torch.sum(weights_dyreg_d[..., None] * semantic_latents_d, -2)
            # semantic_latent_map_s = torch.sum(weights_s[..., None] * semantic_latents_s, -2)
            # semantic_latent_map_full = torch.sum(
            # (T_full * alpha_d * blending)[..., None] * semantic_latents_d + \
            # (T_full * alpha_s * (1. - blending))[..., None] * semantic_latents_s, -2)

            semantic_logit_map_d = net.train_step(weights_d, semantic_latents_d, mode = "semantic_d")
            semantic_logit_map_dyreg_d = net.train_step(weights_dyreg_d, semantic_latents_d, mode = "semantic_d")
            semantic_logit_map_s = net.train_step(weights_s, semantic_latents_s, mode = "semantic_s")
            semantic_logit_map_full = net.train_step(
                T_full, alpha_d, alpha_s, blending, semantic_latents_d, semantic_latents_s, mode = "semantic_full")
        else:
    ...
    def raw2outputs_d(self,
        ...
        if use_semantic_labels:
            ...
            # semantic_logit_map_d = self.forward(weights_d, semantic_latents_d, semantic_func=semantic_mode)
            if semantic_mode == "semantic_s":
                semantic_logit_map_d = self.semantic_static(weights_d, semantic_latents_d,)
            elif semantic_mode == "semantic_d":
                semantic_logit_map_d = self.semantic_dynamic(weights_d, semantic_latents_d,)
        
            # semantic_logit_map_d = self.train_step(weights_d, semantic_latents_d, mode=semantic_mode)
        else:
            semantic_logit_map_d = None
    ...
    def forward(self, *inputs, **kwargs):
        if "semantic_func" not in kwargs.keys():
            return self.forward_ray(*inputs, **kwargs)
        elif "semantic_func" in kwargs.keys():
            func = getattr(self, kwargs["semantic_func"])
            return func(*inputs, **kwargs)

    def forward_ray(self, xyzt_ndc, ndc_wo_embed, use_network='dynamic', feature_dict=None, coarse=True, viewdirs=None, far=False, mode="forward", chain_5frames=False, embed_fn=None):
        """
