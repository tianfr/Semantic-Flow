...
class ResnetFC_static(nn.Module):
    def __init__(
        ...
        if self.use_semantic_labels:
            ...
            # self.semantic_adjust = nn.Sequential(
    ...
    def forward(self, x, combine_inner_dims=(1,), combine_index=None, dim_size=None):
        ...
        with profiler.record_function('resnetfc_infer'):
            ...
            if self.use_semantic_labels:
                semantic_labels = self.semantic_layers(x_)
                return torch.cat([out, blending, semantic_labels], dim=-1)
                # return torch.cat([out, blending, self.semantic_adjust(x_)], dim=-1)
