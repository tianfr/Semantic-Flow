                    ...
                    with torch.no_grad():
                        ...
                        for b in range(len(img_ies)):
                            ...
                            if self.args.no_ndc:
                                ...
                                induced_flow_f = induce_flow(H[b].cuda(), W[b].cuda(), focal[b].cuda(), pose_f, ret['weights_d'][b], ret['raw_pts_f'][b], grid[b, ..., :2])
                            ...
                            if self.args.no_ndc:
                                ...
                                induced_flow_b = induce_flow(H[b].cuda(), W[b].cuda(), focal[b].cuda(), pose_b, ret['weights_d'][b], ret['raw_pts_b'][b], grid[b, ..., :2])

                            induced_flow_f_img = flow_to_image(induced_flow_f.cpu().numpy())
                            induced_flow_b_img = flow_to_image(induced_flow_b.cpu().numpy())

                            psnr = mse2psnr(img2mse(ret['rgb_map_full'][b], target[b]))
