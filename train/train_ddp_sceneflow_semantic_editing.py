    ...
    def train_step(self, data, global_step):
        ...
        with logging_redirect_tqdm():
            ...
            for i in tqdm(range(self.start, self.args.N_iters)):
                ...
                # import ipdb; ipdb.set_trace()
                # right = feature_dict["latent_dict"]['latent'][...,:,-94:]
                # import ipdb; ipdb.set_trace()
                # left = feature_dict["latent_dict"]['latent'][...,:,:-94]
                # tmp = F.interpolate(feature_dict["latent_dict"]['latent'], (12, 135, 240))
                # feature_dict["latent_dict"]['latent'] = torch.zeros_like(feature_dict["latent_dict"]['latent']).cuda()
                # feature_dict["latent_dict"]['latent'][..., :, :94] = right
                # feature_dict["latent_dict"]['latent'][..., :, 94:] = left
                # feature_dict["latent_dict"]['latent'][..., :, -94:] = right
                # feature_dict["latent_dict"]['latent'][..., :-50] = feature_dict["latent_dict"]['latent'][..., 50:]
                # feature_dict["latent_dict"]['latent'] = F.interpolate(feature_dict["latent_dict"]['latent'][...,60:,10:], (12, 240, 135))
                # if self.conf.get_bool("info.change_background", False) and flip_flag:
                #     # flip_flag = False

                #     # if self.conf.get_bool("info.flip_foreground", False):
                #     #     print("WARNING: FLIPPING FEATURES.")
                #     #     feature_dict['latent_dict']['latent'] = feature_dict['latent_dict']['latent'].flip(4)
                #     # else:
                #     #     feature_dict['latent2d_dict']['latent'] = feature_dict['latent2d_dict']['latent'].flip(0)
                #     # feature_dict['latent2d_dict']['latent'] = feature_dict['latent2d_dict']['latent'].flip(0)
                #     feature_dict['latent_dict']['latent'] = feature_dict['latent_dict']['latent'].flip(0)
                    ...
                    if self.conf.get_bool("model.use_semantic_labels", False):
                        ...
                        if self.conf.get_bool("info.use_semantic_consistency", False):
                            ...
                            if img_i < num_img - 1:
                                ...
                                loss += self.conf.get_float('info.semantic_consistency_lambda', 0.1) * loss_dict['semantic_logits_d_f']
                            ...
                            if img_i > 0:
                                ...
                                loss += self.conf.get_float('info.semantic_consistency_lambda', 0.1) * loss_dict['semantic_logits_d_b']
