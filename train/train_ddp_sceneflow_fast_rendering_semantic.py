        ...
        if self.args.pretrain:
            ...
            with logging_redirect_tqdm():
                ...
                # for i in tqdm(range(5)):
                    ...
                    if self.args.fast_render_iter != 0:
                        loss.backward()
                        self.optimizer.step()
        ...
        with logging_redirect_tqdm():
            ...
            for i in tqdm(range(self.start, self.args.N_iters)):
                ...
                if self.conf.get_bool("info.change_background", False):
                    print("WARNING: CHANGING BACKGROUND.")
                elif self.args.fast_render_iter != 0:
                    loss.backward()
                ...
                # Rest is logging
                if i % self.args.i_weights==0 and (i > 0 or self.args.fast_render_iter==0) and self.rank == 0:
                ...
                if i % self.args.i_video == 0 and self.rank == 0 and (i > 0 or self.args.fast_render_iter==0):
                    ...
                    moviebase = os.path.join(
                        testsavedir, '{}_{}_{:06d}_'.format(self.expname, result_type, i))
                    save_res(moviebase, ret, images.shape[0])
                if i % self.args.i_testset == 0 and self.rank == 0 and (i > 0 or self.args.fast_render_iter==0):
                ...
                if i % self.args.i_testset_all == 0 and self.rank == 0 and (i > 0 or self.args.fast_render_iter==0) and self.conf.get_bool("info.testset_all", False):
